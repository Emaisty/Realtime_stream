...
static int h264dec_new(h264dec_t *h, uint8_t *data, int len) {
	h->c = avcodec_find_decoder(AV_CODEC_ID_H264); // CODEC_ID_H264
...
func (m *H264Decoder) Decode(nal []byte, ratio image.YCbCrSubsampleRatio) (f *image.YCbCr, err error) {
	r := C.h264dec_decode(
		&m.m,
	...
	if ratio == image.YCbCrSubsampleRatio444 {
		f = &image.YCbCr{
			...
			Cb:             fromCPtr(unsafe.Pointer(m.m.f.data[1]), cs*h),
			Cr:             fromCPtr(unsafe.Pointer(m.m.f.data[2]), cs*h),
			YStride:        ys,
			CStride:        cs,
			SubsampleRatio: image.YCbCrSubsampleRatio444,
			Rect:           image.Rect(0, 0, w, h),
		}
	} else if ratio == image.YCbCrSubsampleRatio420 {
		f = &image.YCbCr{
			Y:              fromCPtr(unsafe.Pointer(m.m.f.data[0]), ys*h),
